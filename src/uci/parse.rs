...
fn parse_and_push(caps: &Captures, name: &str, constructor: fn(u64) -> GoOption, options: &mut Vec<GoOption>) {
...
impl Uci {
    pub fn parse(&self, cmd: &str) -> UciCommand {
        ...
        let fen_regex = Regex::new("position fen (?<fen>.*?)(?=\\smoves|$)(?: moves (?<moves>.*))?").unwrap();
